...
<body>
	...
	<div class="container mt-5">
		...
		<form action="GradeController" method="get">
			<input type="hidden" name="id"
				value="<%=request.getParameter("id") == null ? "" : request.getParameter("id")%>" />
			<div class="form-group">
				<label for="input-name">Type</label> <input type="text" name="type"
					value="<%=request.getParameter("type") == null ? "" : request.getParameter("type")%>"
					class="form-control" id="type"
					placeholder="Entrez le type de garde">
			</div>
		...
		<table class="table">
			...
			<tbody>
				...
				<%
				GardeService vs = new GardeService();
				for (Garde v : vs.findAll()) {
				%>
				<tr>
					<td><%=v.getId()%></td>
					<td><%=v.getType()%></td>
					<td><a href="GradeController?id=<%=v.getId()%>&op=update"
						class="btn btn-primary btn-sm mr-2"> <i class="fas fa-edit"></i>
							Modifier
					</a> <a href="GradeController?id=<%=v.getId()%>&op=delete"
